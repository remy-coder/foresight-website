...
with open('src/App-RemyDP.tsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
about_hero_old = """        <div className="max-w-5xl mb-16 md:mb-24 relative z-10">
          <motion.div
...
about_hero_new = """        <div className="max-w-5xl mb-16 md:mb-24 relative z-10">
          <motion.div
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-primary/5 border border-primary/10 text-primary text-xs font-black uppercase tracking-[0.2em] mb-10 shadow-sm"
          ...
          <motion.p
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2, duration: 0.8 }}
            className="text-lg md:text-2xl text-gray-500 leading-relaxed font-display font-medium max-w-4xl italic"
          >
...
projects_page_old = """            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
              {filteredProjects.map((project) => (
                <motion.div"""
projects_page_new = """            <div className="mb-12">
...
projects_page_new_end = """              ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
                ...
                >
                  <div className="relative h-64 overflow-hidden grayscale opacity-80">
                    ...
                    />
                    <div className="absolute top-6 left-6 px-4 py-2 bg-white/90 backdrop-blur-md rounded-xl text-gray-600 font-display font-black uppercase tracking-widest text-[11px]">
                      {project.location}
                    </div>
                  </div>
        ...
        function ProjectDetailPage"""
content = content.replace(projects_page_old_end, projects_page_new_end)

content = content.replace('SECTION 3: What we do', 'SECTION 3: Our Work')
content = content.replace('What we do</h3>', 'Our Work</h3>')
content = content.replace('SECTION 4: Key achievements', 'SECTION 4: Stats')
content = content.replace('Key achievements</h3>', 'Stats</h3>')
content = content.replace('SECTION 2: Why it matters', 'SECTION 2: Impact')
content = content.replace('Why it matters</h3>', 'Impact</h3>')
...
content = content.replace('grid-cols-1 md:grid-cols-3', 'grid-cols-1 md:grid-cols-2 lg:grid-cols-4')

# News Page Update
news_page_old = """            function NewsPage() {
  return (
...
news_page_new = """            function NewsPage() {
  return (
...
content = content.replace(news_page_old, news_page_new)

# Reduce whitespace:
content = content.replace('py-24 md:py-32', 'py-16 md:py-24')
content = content.replace('mb-32 md:mb-48', 'mb-16 md:mb-24')
content = content.replace('py-24 md:py-40', 'py-16 md:py-24')
content = content.replace('text-[13px]', 'text-[11px]')

# Navigation update Impact -> Stories from the Field
# Wait, this is used in the nav menu, we need to update it in App-RemyDP.tsx if it's there
# It's actually in constants.ts. I update constants.ts below.

with open('src/App-RemyDP.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
